...
Generated 2026-09-17 by `python eval/report.py`.
...
|---|---|---|---|---|---|---|---|
| primary (commercial) | 126 | 100% | 100% | 100% | 100% | 14.6s | 65.0 hal |
| on-prem (vLLM route) | 126 | 94% | 92% | 95% | 100% | 24.9s | 26.0 hal |
| cheap (small model) | 126 | 95% | 95% | 95% | 100% | 10.5s | 2.9 hal |
...
- Guard latency: {'deterministic': 0.07, 'classifier': 70.29, 'pii': 0.02} (milliseconds, by layer).
...
|---|---|---|---|---|---|
| Baseline (`answer_faq.v4`, no cache, no routing) | 4.09 hal | +0% | 184 ms | 1167 ms | 55% |
| + prompt-cache discipline (`answer_faq.v5`) | 3.00 hal | -27% | 179 ms | 1111 ms | 71% |
| + response cache (exact + semantic) | 1.66 hal | -59% | 135 ms | 858 ms | 66% |
| + routing table | 0.37 hal | -91% | 123 ms | 712 ms | 65% |
